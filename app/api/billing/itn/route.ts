...
async function handleCallback(request: NextRequest, payload: Record<string, string>) {
  ...
  if (redirectMode === 'Y') {
    return NextResponse.redirect(new URL(success ? `${process.env.NEXT_PUBLIC_APP_URL}/billing?success=1` : `${process.env.NEXT_PUBLIC_APP_URL}/billing/suspended?failed=1`));
  }
