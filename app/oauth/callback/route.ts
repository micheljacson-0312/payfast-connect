...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return applySessionCookie(
      NextResponse.redirect(getAppUrlWithSearch(`/settings?installed=1&username=${encodeURIComponent(defaultUsername)}&password=${encodeURIComponent(defaultPassword)}`, request)),
      locationId
    );
  } catch (err) {
